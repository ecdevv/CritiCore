...
const CardGrid = ({ data = [], children }: { data?: CardCategories[], children?: React.ReactNode }) => {
  ...
  <div className='w-full flex flex-col justify-center items-center gap-5'>
    <div className={`relative sm:block flex flex-col gap-3 w-full text-3xl font-bold text-white tracking-wide ${children ? 'sm:text-left' : 'lg:text-left'} text-center`}><h1>{data[0].category}</h1>{children}</div>
